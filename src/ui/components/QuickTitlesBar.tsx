    ...
    return (
        <div className="app-quick-titles flex justify-between items-start items-center gap-2 flex-wrap bg-gray-50 p-2 rounded">
            <div className="flex gap-2">
                {/* QuickTitle buttons */}
                {quickTitles.map((qt) => (
                    <div className="app-quick-title flex rounded-lg border border-blue-700 overflow-hidden" key={qt}>
                        <button
                            onClick={() => {
                                onApplyPrefix(qt)
                                focusEditor()
                            }}
                            className="app-button app-button-primary bg-blue-600 text-white px-3 py-2 rounded text-sm hover:bg-blue-700"
                        >
                            {qt.toUpperCase()}
                        </button>
                        {editMode && (
